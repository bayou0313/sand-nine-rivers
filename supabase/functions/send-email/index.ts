...
function orderCustomerEmail(order: any, feePercent = 3.5, feeFixed = 0.30, pricingMode = "transparent"): string {
  ...
  const baseLine = isBaked
    ? (baseUnitPrice + rawDistanceFee / qty) * qty
    : (baseUnitPrice > 0 ? baseUnitPrice * qty : 0);
  const distanceFee = isBaked ? 0 : rawDistanceFee;
...
<html lang="en">
  <head>
    ...
    <body style="margin:0;padding:0;background-color:#F0EDE5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
          ...
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            ...
            <tr>
              <td style="background-color:#FFFFFF;padding:32px;">
                ...
                <!-- Greeting -->
                ${companyName ? `
                <p style="margin:0 0 2px;font-size:14px;font-weight:700;color:${BRAND_COLOR};">
                  ${companyName}
                </p>` : ''}
                  ...
                  <tr>
                    <td style="padding:16px 24px;border-top:1px solid #E8E5DC;">
                       ...
                       <p style="font-size:12px;color:#666666;margin:0;line-height:1.6;">
                         ${isStripePaid ? "Cancel anytime before your driver is on route — you will not be charged. Once your driver is on route, your delivery is confirmed and non-refundable." : "Cancel anytime before your driver is on route. Your order will simply be removed — no payment was collected."}
                       </p>
                    </td>
                  </tr>
                </table>
